...
### Personal landing-page discipline

For Kenneth's personal website / landing page explorations, default to a sparse one-screen surface unless he asks for a broader site:
- no blog archive, timeline, dashboard, metrics, section grids, or invented personal data;
- only a name, concise tagline, public links, optional email/contact, and possibly one subtle public location if explicitly provided;
- public feed hints (Instagram, X, GitHub, occasional article) should be small/link-level accents, not content-heavy sections;
- if using a generated background image as guide, preserve it as the hero/background and add typography/links with a light touch rather than rebuilding the whole visual language.
...
### Personal landing pages with low available content

When designing a personal landing page for Kenneth and he has not supplied much content, keep the page intentionally sparse instead of inventing sections to fill space.

Default shape:
- one screen or nearly one screen;
- name, one-line positioning, and a few outbound links;
- optional small public feed hints (Instagram, X, GitHub, latest article) only when requested or clearly helpful;
- no detailed biography, timeline, metrics, location, private travel details, fake dashboards, or large content grids.

For split visual concepts (human/machine, public/private, taste/systems), don't assume a high-contrast black/white split. Prefer a unified palette and same-color typography, using texture, material, linework, composition, or motion to distinguish the sides while preserving legibility.
